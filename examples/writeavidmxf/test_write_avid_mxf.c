    ...
    unsigned char** dataOut, long* dataOutSize, int* haveImage)
{
    *haveImage = 0;
    ...
    while (!(*haveImage) && state->position < state->dataSize)
    {
        ...
        if (state->endOfField)
        {
            /* mjpeg151s frame is a single field; other mjpeg frame are 2 fields */
            if (state->resolution == Res151 || state->field2)
            {
                *haveImage = 1;
            }
            state->endOfField = 0;
...
static void usage(const char* cmd)
{
    ...
    fprintf(stderr, "       --res <resolution>  Resolution '2:1' (default), '3:1', '10:1', '15:1' or '20:1'\n");
    ...
    fprintf(stderr, "  --unc <filename>         Uncompressed 8-bit UYVY\n");
...
int main(int argc, const char* argv[])
{
    ...
    while (cmdlnIndex < argc)
    {
        ...
        else if (strcmp(argv[cmdlnIndex], "--res") == 0)
        {
            ...
            else if (strcmp(argv[cmdlnIndex + 1], "15:1") == 0)
            {
                inputs[inputIndex - 1].essenceInfo.avidMJPEGInfo.resolution = Res151;
            }
    ...
    while (!done)
    {
        for (i = 0; i < inputIndex; i++)
        {
            ...
            else if (inputs[i].essenceType == UncUYVY)
            {
                ...
                if (!write_samples(clipWriter, inputs[i].materialTrackID, 1, inputs[i].buffer, inputs[i].frameSize))
                {
                    fprintf(stderr, "Failed to write UncUYVY frame\n");
                    goto abort;
                }
            }
            else if (inputs[i].essenceType == PCM && !inputs[i].isWAVFile)
