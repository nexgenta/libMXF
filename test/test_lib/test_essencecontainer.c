...
int test_read(const char* filename)
{
    ...
    CHK_OFAIL(mxf_read_essence_element_data(mxfFile, essenceElement, len,
        essenceData, &numRead));
    ...
    CHK_OFAIL(mxf_open_essence_element_read(mxfFile, &key, llen, len, &essenceElement));
    CHK_OFAIL(llen == 8 && len == 1024);
    CHK_OFAIL(mxf_read_essence_element_data(mxfFile, essenceElement, 256,
        essenceData, &numRead));
    CHK_OFAIL(numRead == 256);
    CHK_OFAIL(mxf_read_essence_element_data(mxfFile, essenceElement, 768,
        &essenceData[256], &numRead));
...
int test_create_and_write(const char* filename)
{
    ...
    CHK_OFAIL(mxf_open_essence_element_write(mxfFile, &g_bwfClipWrappedEEKey,
        8, 1024, &essenceElement));
